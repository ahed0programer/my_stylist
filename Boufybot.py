phrases = {
    "greets":[
        "Hi🌟 ,it's a great day for stunning apperiance , what do we want to wear tody",
        "Hi 😊 ,wondering what to wear?! no worries , Boufy's here with you",
        "Hey there! 🌟 Ready to slay the day with a fabulous outfit? Let's get started!",
        "Hello! 👗 Need some outfit inspiration? Boufy's got you covered!",
        "Hi there! 😊 Excited to find the perfect look for today? Let's dive in!",
        "Greetings! 🎉 Let's pick out something amazing for you to wear today!",
        "Hi! 👋 Ready to look your best? I'm here to help you find the perfect outfit!",
        "Hello! 🌼 Wondering what to wear? Boufy's here to make it easy for you!",
        "Hi! 👗 Feeling stylish? Let's choose something fantastic to wear today!",
        "Hey! 😎 Ready to shine? Let's find you an outfit that stands out!",
        "Hello! 🥳 Looking for some style advice? Boufy is here to assist you!",
        "Hi! 🌟 Ready to dress to impress? Let's get started on your outfit!"
    ],
    "collect_data":{
        # * informations need to bee asked 
        # ** ["prefered_color", "shirt_size" ,"pants_size" ,"shoe_size" , "age" , "hieght"]
        "prefered_color":[
            "what colors 👗 do you like",
            "what colors 👗 do you prefer",
            "what colors👗 you think make you feel comfortable",
            "you have a taste 🌟,tell me your favourite color",
        ],
        "shirt_size":[
            "what's size of shirts 👗 you normally wear",
            "what's size of shirts 👗 fits you well",
            "what's the size of your shirts , so I can prvide you better assistance",
        ],
        "pants_size":[
            "what's size of pants you normally wear 😊",
            "what's size of pants fits you well 😊",
            "what's the size of your pants , so I can prvide you better assistance 😊",
        ],
        "shoe_size":[
            "what's size of shoes you normally wear",
            "what's size of shoes fits you well",
            "what's the size of your shoes , so I can prvide you better assistance",
        ],
        "age":[
            "what's your age 😊",
            "pardon me , could you tell me your age 😊",
            "it will be great if you told me your age 😊",
        ],
        "hieght":[
            "how much hieght are you ... in cm 🌟",
            "what is your hieght ... in cm 🌟",
        ],
    },
    "ask_occasion":[
        "So , Do we have any occasions today? party , wedding , ..  🎉🎉",
        "what is our agnenda today 🌟",
        "well , what're we going to do to day , please tell me we're having a party 🥳 I am so excited",
        "have amazing stuff today ?! , 🌼 what is waiting for us?"
    ],
    "ask_taste":[
        "what type of clothing you like",
        "you have a type you love to wear!! "
    ]
}

occasions = {
    "wedding":[
        "family member's wedding",
        "relative's wedding",
        "friend's wedding",
        "acquaintance's wedding"
    ],
    "party": [
        "graduation party",
        "birthday party",
        "just party",
    ],
    "date":[
        "first date crush",
        "date with a girlfriend",
        "date with fiancee"
    ],
    "meeting":[
        "friends meeting",
        "family meeting",
        "job interview",
    ],
    "nothing":[
        "going to work",
        "stay indoor",
        "outdoor entertaiment",
        "going to gym",
        "practicing sport"
    ],
}
